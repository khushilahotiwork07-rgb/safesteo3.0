...
def render():
    ...
    with col_prof:
        # Morse Calculation
        from scoring import calculate_morse_score, RISK_COLORS
        p_morse_score, p_morse_level = calculate_morse_score(patient)
        morse_color = RISK_COLORS.get(p_morse_level, "#3DAA6F")
...
<tr><td style="width:130px;color:{TEXT_GREY};">Morse Score:</td><td style="font-weight:700;color:{morse_color};">{p_morse_score} / 125 ({p_morse_level})</td></tr>
...
"""
        ...
        # ── Persistent Morse Fall Scale Assessment ──
        st.markdown("<div style='height:0.5rem;'></div>", unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown(f"<div style='font-size:0.85rem;font-weight:700;color:{TEXT_DARK};margin-bottom:0.5rem;'>Morse Fall Scale Assessment</div>", unsafe_allow_html=True)
            
            from database import update_patient_morse
            
            m_hist = st.checkbox("History of falling (25)", value=patient.get("morse_history", False), key=f"mh_{chosen_id}")
            m_second = st.checkbox("Secondary diagnosis (15)", value=patient.get("morse_secondary", False), key=f"ms_{chosen_id}")
            
            aid_opts = ["None (0)", "Cane/Walker (15)", "Furniture (30)"]
            m_aid = st.selectbox("Ambulatory Aid", aid_opts, index=0 if "none" in str(patient.get("morse_aid")).lower() else 1 if "furniture" not in str(patient.get("morse_aid")).lower() else 2, key=f"ma_{chosen_id}")
            
            m_iv = st.checkbox("IV/Heparin Lock (20)", value=patient.get("morse_iv", False), key=f"mi_{chosen_id}")
            
            gait_opts = ["Normal (0)", "Weak (10)", "Impaired (20)"]
            m_gait = st.selectbox("Gait", gait_opts, index=0 if "normal" in str(patient.get("morse_gait")).lower() else 1 if "weak" in str(patient.get("morse_gait")).lower() else 2, key=f"mg_{chosen_id}")
            
            ment_opts = ["Oriented (0)", "Forgets limitations (15)"]
            m_mental = st.selectbox("Mental Status", ment_opts, index=0 if "oriented" in str(patient.get("morse_mental")).lower() else 1, key=f"mm_{chosen_id}")

            # Check if any Morse values changed
            current_morse = {
                "history": m_hist, "secondary": m_second, "aid": m_aid,
                "iv": m_iv, "gait": m_gait, "mental": m_mental
            }
            
            if any(current_morse[k] != patient.get(f"morse_{k}") for k in ["history", "secondary", "aid", "iv", "gait", "mental"]):
                update_patient_morse(chosen_id, current_morse)
                st.toast(f"Morse Fall Scale updated: {patient['name']}", icon="📋")
                st.rerun()
