    ...
    try:
        ...
        conn.execute('''
            CREATE TABLE IF NOT EXISTS patients (
                ...
                morse_history INTEGER DEFAULT 0,
                morse_secondary INTEGER DEFAULT 0,
                morse_aid TEXT DEFAULT 'None',
                morse_iv INTEGER DEFAULT 0,
                morse_gait TEXT DEFAULT 'Normal',
                morse_mental TEXT DEFAULT 'Oriented'
            )
        ''')
        ...
        for col in ["morse_history", "morse_secondary", "morse_iv"]:
            try:
                conn.execute(f"ALTER TABLE patients ADD COLUMN {col} INTEGER DEFAULT 0")
            except sqlite3.OperationalError: pass
        for col in ["morse_aid", "morse_gait", "morse_mental"]:
            try:
...
def update_patient_morse(patient_id: int, morse_data: dict):
    conn = _get_connection()
    try:
        conn.execute('''
            UPDATE patients
            SET morse_history=?, morse_secondary=?, morse_aid=?, 
                morse_iv=?, morse_gait=?, morse_mental=?
            WHERE patient_id=?
        ''', (
            1 if morse_data.get('history') else 0,
            1 if morse_data.get('secondary') else 0,
            morse_data.get('aid', 'None'),
            1 if morse_data.get('iv') else 0,
            morse_data.get('gait', 'Normal'),
            morse_data.get('mental', 'Oriented'),
            patient_id
        ))
        conn.commit()
    except Exception as e:
        print(f"Error updating Morse data: {e}")
    finally:
        conn.close()
    ...
    try:
        conn.execute('''
            INSERT INTO patients (
                ...
                morse_history, morse_secondary, morse_aid,
                morse_iv, morse_gait, morse_mental
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            ...
            1 if p.get('morse_history') else 0, 1 if p.get('morse_secondary') else 0,
            p.get('morse_aid', 'None'), 1 if p.get('morse_iv') else 0,
            p.get('morse_gait', 'Normal'), p.get('morse_mental', 'Oriented')
        ))
        conn.commit()
